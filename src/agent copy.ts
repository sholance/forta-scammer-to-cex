...
import { ALERT_ERC20_ASSET_DEPOSIT, CEX_ADDRESSES, CEX_ADDRESSES_v2 } from './constants';
  ...
  try {
    ...
    if (cexList.has(cexDepositAddress) || CEX_ADDRESSES.includes(cexDepositAddress) || CEX_ADDRESSES_v2.includes(cexDepositAddress)) {
      ...
      if (isFromScammer) {
        ...
        const chainId = getEthersProvider()
        findings.push(
          Finding.fromObject({
            name: 'Known Scammer Asset Deposit',
            description: `Known scammer ${from} deposited ${txValue} to CEX ${cexDepositAddress} ${cexInfo.name}`,
...
function getCexInfo(address: string): { name: string } {
  return addresses2.find((item) => item.to_address === address) || { name: '' };
}
